          ...
          rich: {
            ...
            score: {
              color: function(params) {
                const index = indicators.findIndex(item => item.name === params.name);
                const item = data[index];
                return getColorByValue(item.value, item.min, item.max, item.low, item.high);
              },
              fontSize: 12,
              fontWeight: 'bold',
              padding: [5, 0, 0, 0] // 从[2, 0, 0, 0]增加到[5, 0, 0, 0]，增加上边距
            }
          }
        },
        splitLine: {
